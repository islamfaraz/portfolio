...
\begin{itemize}
    ...
    \item Architected multi-region API platform with Azure APIM and Front Door, routing traffic to region-specific endpoints via geolocation-based load balancing and securing backends with Entra ID OAuth 2.0 client credentials.
    \item Designed secure CI/CD pipelines with SAST/DAST integration and security gates, reducing vulnerability escape rate by 60\%.
    ...
    \item Hardened Kubernetes clusters with Pod Security Standards, Network Policies, and RBAC, achieving 90\% compliance with CIS benchmarks.
    ...
    \item Implemented secrets management using HashiCorp Vault and AWS Secrets Manager, eliminating hard-coded credentials across 30+ repositories.
...
\begin{itemize}
    ...
    \item Deployed Azure Sentinel SIEM with Logic Apps for automated incident response, cutting mean detection time by 40\%.
    ...
    \item Built centralised security logging infrastructure using ELK Stack and FluentBit, consolidating logs from 50+ services.
...
\begin{itemize}
    ...
    \item Secured APIM backend APIs with Entra ID and OAuth 2.0 client credentials flow, enforcing zero-trust authentication for all service-to-service communication.
    \item Implemented container image scanning and vulnerability remediation workflows, reducing critical CVEs by 70\%.
...
\begin{itemize}
    ...
    \item Containerised C\# microservices as Docker images and deployed them on Azure AKS, exposing APIs through Azure API Management across 3 regions for high availability.
    \item Developed microservices and cloud-native applications, lowering service latency and improving scalability.
    \item Architected multi-region API gateway using Azure Front Door as a global load balancer, routing traffic to region-specific APIM instances based on user geolocation and reducing API latency by 40\%.
    \item Integrated vulnerability scanning into CI/CD pipelines, establishing early detection of security issues and reducing post-deployment remediation by 35\%.
...
\cvtag{Helm}
\cvtag{Service Mesh}
\cvtag{Pod Security Standards}
\cvtag{K8s Network Policies}
...
\divider
\textbf{\textcolor{maroon}{Security \& DevSecOps:}}\\
\cvtag{SAST / DAST}
\cvtag{Azure Sentinel}
\cvtag{AWS GuardDuty}
\cvtag{AWS Security Hub}
\cvtag{HashiCorp Vault}
\cvtag{AWS Secrets Manager}
\cvtag{Container Security}
\cvtag{Vulnerability Scanning}
\cvtag{Cloudflare Zero Trust}
...
\wheelchart{1.0cm}{0.4cm}{%
  6/5em/accent!30/{Sleep,\\beautiful sleep},
  3/5em/accent!40/Hopeful novelist by night,
  8/5em/accent!60/Actively learning \& upskilling,
  2/5em/accent/Sports \& relaxation,
  5/5em/accent!20/Spending time with family
}
